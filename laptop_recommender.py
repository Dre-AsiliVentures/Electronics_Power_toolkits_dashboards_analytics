...
if intel_core_version != 1 or fingerprint_face_id or ram != 8 or internal_storage != 512 or price != 1000:
    filtered_df = df[(df['Intel Core Version'] == intel_core_version) & (df['Fingerprint/Face ID Availability'] == 'Yes') & (df['RAM Storage Capacity'] >= ram) & (df['Internal Storage'] >= internal_storage) & (df['Price'] <= price)]
